...
class carSystem{
    ...
    carSystem(string name1, string name2, double price, bool st, string id) 
        : brand(name1), model(name2), rentalPrice(price), status(st), totalRevenue(0.0), carID(id) {}

    carSystem(const carSystem &c) 
        : brand(c.brand), model(c.model), rentalPrice(c.rentalPrice), status(true), totalRevenue(0.0), carID("COPY") {}
