...
protected:
    ...
    bool feePaid;
public:
    person(string i, string n, string stop, bool fee)
        : id(i), name(n), assignedStop(stop), feePaid(fee) {}
    ...
    bool getFeePaid() const { return feePaid; }
};

class Teacher : public person {  
public:
    Teacher(string i, string n, string stop) 
        : person(i, n, stop, false) {}
...
public:
    ...
    void payTeacherFee(string id, int months) {
        for(int i = 0; i < teacherCount; i++) {
            if(teachers[i]->getId() == id) {
                teachers[i]->payFees(months);
                return;
            }
        }
...
int main() {
    ...
    system.payStudentFee("S1");
    system.payTeacherFee("T1", 1);
